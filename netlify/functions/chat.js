...
export async function handler(event) {
  ...
  try {
    ...
    // Usa un modelo rápido (gemini-2.5-flash) para chat
    const model = genAI.getGenerativeModel({ model: 'gemini-2.5-flash' });

    // Contexto: ahora la aplicación sólo compara dos plataformas
    const chatContext = `
      Eres un asistente experto en sistemas SCADA. Tu conocimiento se basa en la
      información de una aplicación web que compara 2 plataformas:
      Zenon COPADATA y Power Operation Schneider.
    ...
    const chat = model.startChat({
      history: [
        { role: 'user', parts: [{ text: chatContext }] },
        {
          role: 'model',
          parts: [
            { text: 'Entendido. Soy un asistente experto en SCADA. ¿En qué puedo ayudarte?' }
          ]
        }
      ]
    });
